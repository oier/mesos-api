...
module Mesos.Protos.CapabilityInfo (CapabilityInfo(..)) where
import Prelude ((+), (/))
...
import qualified Mesos.Protos.CapabilityInfo.Capability as Protos.CapabilityInfo (Capability)
...
instance P'.ReflectDescriptor CapabilityInfo where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".mesos.CapabilityInfo\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\"], baseName = MName \"CapabilityInfo\"}, descFilePath = [\"Mesos\",\"Protos\",\"CapabilityInfo.hs\"], isGroup = False, fields = fromList [FieldInfo {fieldName = ProtoFName {protobufName' = FIName \".mesos.CapabilityInfo.capabilities\", haskellPrefix' = [MName \"Mesos\"], parentModule' = [MName \"Protos\",MName \"CapabilityInfo\"], baseName' = FName \"capabilities\", baseNamePrefix' = \"\"}, fieldNumber = FieldId {getFieldId = 1}, wireTag = WireTag {getWireTag = 8}, packedTag = Just (WireTag {getWireTag = 8},WireTag {getWireTag = 10}), wireTagLength = 1, isPacked = False, isRequired = False, canRepeat = True, mightPack = True, typeCode = FieldType {getFieldType = 14}, typeName = Just (ProtoName {protobufName = FIName \".mesos.CapabilityInfo.Capability\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\",MName \"CapabilityInfo\"], baseName = MName \"Capability\"}), hsRawDefault = Nothing, hsDefault = Nothing}], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"

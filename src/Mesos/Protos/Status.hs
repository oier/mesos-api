...
module Mesos.Protos.Status (Status(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Status where
  ...
  toEnum = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.Status") . toMaybe'Enum
  succ DRIVER_NOT_STARTED = DRIVER_RUNNING
  succ DRIVER_RUNNING = DRIVER_ABORTED
  succ DRIVER_ABORTED = DRIVER_STOPPED
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.Status"
  pred DRIVER_RUNNING = DRIVER_NOT_STARTED
  pred DRIVER_ABORTED = DRIVER_RUNNING
  pred DRIVER_STOPPED = DRIVER_ABORTED
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.Status"
...
instance P'.ReflectEnum Status where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".mesos.Status") ["Mesos"] ["Protos"] "Status") ["Mesos", "Protos", "Status.hs"]

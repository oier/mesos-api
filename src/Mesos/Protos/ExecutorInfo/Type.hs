...
module Mesos.Protos.ExecutorInfo.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  fromEnum CUSTOM = 2
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.ExecutorInfo.Type") . toMaybe'Enum
  succ UNKNOWN = DEFAULT
  succ DEFAULT = CUSTOM
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.ExecutorInfo.Type"
  pred DEFAULT = UNKNOWN
  pred CUSTOM = DEFAULT
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.ExecutorInfo.Type"
...
instance P'.ReflectEnum Type where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".mesos.ExecutorInfo.Type") ["Mesos"] ["Protos", "ExecutorInfo"] "Type")
      ["Mesos", "Protos", "ExecutorInfo", "Type.hs"]

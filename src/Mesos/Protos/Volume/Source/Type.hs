...
module Mesos.Protos.Volume.Source.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  fromEnum SECRET = 3
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.Volume.Source.Type") . toMaybe'Enum
  succ UNKNOWN = DOCKER_VOLUME
  succ DOCKER_VOLUME = HOST_PATH
  succ HOST_PATH = SANDBOX_PATH
  succ SANDBOX_PATH = SECRET
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.Volume.Source.Type"
  pred DOCKER_VOLUME = UNKNOWN
  pred HOST_PATH = DOCKER_VOLUME
  pred SANDBOX_PATH = HOST_PATH
  pred SECRET = SANDBOX_PATH
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.Volume.Source.Type"
...
instance P'.ReflectEnum Type where
  reflectEnum
   = [(0, "UNKNOWN", UNKNOWN), (1, "DOCKER_VOLUME", DOCKER_VOLUME), (4, "HOST_PATH", HOST_PATH), (2, "SANDBOX_PATH", SANDBOX_PATH),
      (3, "SECRET", SECRET)]
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".mesos.Volume.Source.Type") ["Mesos"] ["Protos", "Volume", "Source"] "Type")
      ["Mesos", "Protos", "Volume", "Source", "Type.hs"]

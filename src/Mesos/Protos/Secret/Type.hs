...
module Mesos.Protos.Secret.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  fromEnum VALUE = 2
  toEnum = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.Secret.Type") . toMaybe'Enum
  succ UNKNOWN = REFERENCE
  succ REFERENCE = VALUE
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.Secret.Type"
  pred REFERENCE = UNKNOWN
  pred VALUE = REFERENCE
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.Secret.Type"
...
instance P'.ReflectEnum Type where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".mesos.Secret.Type") ["Mesos"] ["Protos", "Secret"] "Type")
      ["Mesos", "Protos", "Secret", "Type.hs"]

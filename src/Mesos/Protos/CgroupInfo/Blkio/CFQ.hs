...
module Mesos.Protos.CgroupInfo.Blkio.CFQ (CFQ(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor CFQ where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".mesos.CgroupInfo.Blkio.CFQ\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\",MName \"CgroupInfo\",MName \"Blkio\"], baseName = MName \"CFQ\"}, descFilePath = [\"Mesos\",\"Protos\",\"CgroupInfo\",\"Blkio\",\"CFQ.hs\"], isGroup = False, fields = fromList [], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"

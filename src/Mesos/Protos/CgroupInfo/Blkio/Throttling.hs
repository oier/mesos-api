...
module Mesos.Protos.CgroupInfo.Blkio.Throttling (Throttling(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor Throttling where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".mesos.CgroupInfo.Blkio.Throttling\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\",MName \"CgroupInfo\",MName \"Blkio\"], baseName = MName \"Throttling\"}, descFilePath = [\"Mesos\",\"Protos\",\"CgroupInfo\",\"Blkio\",\"Throttling.hs\"], isGroup = False, fields = fromList [], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"

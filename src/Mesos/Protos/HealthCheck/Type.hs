...
module Mesos.Protos.HealthCheck.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  fromEnum TCP = 3
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.HealthCheck.Type") . toMaybe'Enum
  succ UNKNOWN = COMMAND
  succ COMMAND = HTTP
  succ HTTP = TCP
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.HealthCheck.Type"
  pred COMMAND = UNKNOWN
  pred HTTP = COMMAND
  pred TCP = HTTP
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.HealthCheck.Type"
...
instance P'.ReflectEnum Type where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".mesos.HealthCheck.Type") ["Mesos"] ["Protos", "HealthCheck"] "Type")
      ["Mesos", "Protos", "HealthCheck", "Type.hs"]

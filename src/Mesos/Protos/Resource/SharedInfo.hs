...
module Mesos.Protos.Resource.SharedInfo (SharedInfo(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor SharedInfo where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".mesos.Resource.SharedInfo\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\",MName \"Resource\"], baseName = MName \"SharedInfo\"}, descFilePath = [\"Mesos\",\"Protos\",\"Resource\",\"SharedInfo.hs\"], isGroup = False, fields = fromList [], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"

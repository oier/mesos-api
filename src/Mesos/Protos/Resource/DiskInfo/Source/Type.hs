...
module Mesos.Protos.Resource.DiskInfo.Source.Type (Type(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Type where
  ...
  fromEnum RAW = 4
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.Resource.DiskInfo.Source.Type") .
      toMaybe'Enum
  succ UNKNOWN = PATH
  succ PATH = MOUNT
  succ MOUNT = BLOCK
  succ BLOCK = RAW
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.Resource.DiskInfo.Source.Type"
  pred PATH = UNKNOWN
  pred MOUNT = PATH
  pred BLOCK = MOUNT
  pred RAW = BLOCK
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.Resource.DiskInfo.Source.Type"
...
instance P'.ReflectEnum Type where
  ...
  reflectEnumInfo _
   = P'.EnumInfo
      (P'.makePNF (P'.pack ".mesos.Resource.DiskInfo.Source.Type") ["Mesos"] ["Protos", "Resource", "DiskInfo", "Source"] "Type")
      ["Mesos", "Protos", "Resource", "DiskInfo", "Source", "Type.hs"]

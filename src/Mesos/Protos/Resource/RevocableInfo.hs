...
module Mesos.Protos.Resource.RevocableInfo (RevocableInfo(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor RevocableInfo where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".mesos.Resource.RevocableInfo\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\",MName \"Resource\"], baseName = MName \"RevocableInfo\"}, descFilePath = [\"Mesos\",\"Protos\",\"Resource\",\"RevocableInfo.hs\"], isGroup = False, fields = fromList [], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"

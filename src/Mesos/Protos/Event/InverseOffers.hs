...
module Mesos.Protos.Event.InverseOffers (InverseOffers(..)) where
import Prelude ((+), (/))
...
import qualified Mesos.Protos.InverseOffer as Protos (InverseOffer)
...
instance P'.ReflectDescriptor InverseOffers where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".mesos.scheduler.Event.InverseOffers\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\",MName \"Event\"], baseName = MName \"InverseOffers\"}, descFilePath = [\"Mesos\",\"Protos\",\"Event\",\"InverseOffers.hs\"], isGroup = False, fields = fromList [FieldInfo {fieldName = ProtoFName {protobufName' = FIName \".mesos.scheduler.Event.InverseOffers.inverse_offers\", haskellPrefix' = [MName \"Mesos\"], parentModule' = [MName \"Protos\",MName \"Event\",MName \"InverseOffers\"], baseName' = FName \"inverse_offers\", baseNamePrefix' = \"\"}, fieldNumber = FieldId {getFieldId = 1}, wireTag = WireTag {getWireTag = 10}, packedTag = Nothing, wireTagLength = 1, isPacked = False, isRequired = False, canRepeat = True, mightPack = False, typeCode = FieldType {getFieldType = 11}, typeName = Just (ProtoName {protobufName = FIName \".mesos.InverseOffer\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\"], baseName = MName \"InverseOffer\"}), hsRawDefault = Nothing, hsDefault = Nothing}], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"

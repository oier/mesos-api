...
module Mesos.Protos.MountPropagation.Mode (Mode(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Mode where
  ...
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.MountPropagation.Mode") .
      toMaybe'Enum
  succ UNKNOWN = HOST_TO_CONTAINER
  succ HOST_TO_CONTAINER = BIDIRECTIONAL
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.MountPropagation.Mode"
  pred HOST_TO_CONTAINER = UNKNOWN
  pred BIDIRECTIONAL = HOST_TO_CONTAINER
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.MountPropagation.Mode"
...
instance P'.ReflectEnum Mode where
  ...
  reflectEnumInfo _
   = P'.EnumInfo (P'.makePNF (P'.pack ".mesos.MountPropagation.Mode") ["Mesos"] ["Protos", "MountPropagation"] "Mode")
      ["Mesos", "Protos", "MountPropagation", "Mode.hs"]

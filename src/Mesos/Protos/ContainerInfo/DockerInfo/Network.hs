...
module Mesos.Protos.ContainerInfo.DockerInfo.Network (Network(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Network where
  ...
  fromEnum USER = 4
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.ContainerInfo.DockerInfo.Network") .
      toMaybe'Enum
  succ HOST = BRIDGE
  succ BRIDGE = NONE
  succ NONE = USER
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.ContainerInfo.DockerInfo.Network"
  pred BRIDGE = HOST
  pred NONE = BRIDGE
  pred USER = NONE
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.ContainerInfo.DockerInfo.Network"
...
instance P'.ReflectEnum Network where
  ...
  reflectEnumInfo _
   = P'.EnumInfo
      (P'.makePNF (P'.pack ".mesos.ContainerInfo.DockerInfo.Network") ["Mesos"] ["Protos", "ContainerInfo", "DockerInfo"] "Network")
      ["Mesos", "Protos", "ContainerInfo", "DockerInfo", "Network.hs"]

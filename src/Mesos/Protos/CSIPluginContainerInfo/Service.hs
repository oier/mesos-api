...
module Mesos.Protos.CSIPluginContainerInfo.Service (Service(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Service where
  ...
  toEnum
   = P'.fromMaybe (Prelude'.error "hprotoc generated code: toEnum failure for type Mesos.Protos.CSIPluginContainerInfo.Service") .
      toMaybe'Enum
  succ UNKNOWN = CONTROLLER_SERVICE
  succ CONTROLLER_SERVICE = NODE_SERVICE
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Mesos.Protos.CSIPluginContainerInfo.Service"
  pred CONTROLLER_SERVICE = UNKNOWN
  pred NODE_SERVICE = CONTROLLER_SERVICE
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Mesos.Protos.CSIPluginContainerInfo.Service"
...
instance P'.ReflectEnum Service where
  ...
  reflectEnumInfo _
   = P'.EnumInfo
      (P'.makePNF (P'.pack ".mesos.CSIPluginContainerInfo.Service") ["Mesos"] ["Protos", "CSIPluginContainerInfo"] "Service")
      ["Mesos", "Protos", "CSIPluginContainerInfo", "Service.hs"]

...
module Mesos.Protos.Call.Reconcile.Task (Task(..)) where
import Prelude ((+), (/))
...
data Task = Task{task_id :: !(Protos.TaskID), slave_id :: !(P'.Maybe Protos.SlaveID)}
...
instance P'.TextMsg Task where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'task_id, parse'slave_id]) P'.spaces

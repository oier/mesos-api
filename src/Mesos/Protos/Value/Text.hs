...
module Mesos.Protos.Value.Text (Text(..)) where
import Prelude ((+), (/))
...
instance P'.ReflectDescriptor Text where
  ...
  reflectDescriptorInfo _
   = Prelude'.read
      "DescriptorInfo {descName = ProtoName {protobufName = FIName \".mesos.Value.Text\", haskellPrefix = [MName \"Mesos\"], parentModule = [MName \"Protos\",MName \"Value\"], baseName = MName \"Text\"}, descFilePath = [\"Mesos\",\"Protos\",\"Value\",\"Text.hs\"], isGroup = False, fields = fromList [FieldInfo {fieldName = ProtoFName {protobufName' = FIName \".mesos.Value.Text.value\", haskellPrefix' = [MName \"Mesos\"], parentModule' = [MName \"Protos\",MName \"Value\",MName \"Text\"], baseName' = FName \"value\", baseNamePrefix' = \"\"}, fieldNumber = FieldId {getFieldId = 1}, wireTag = WireTag {getWireTag = 10}, packedTag = Nothing, wireTagLength = 1, isPacked = False, isRequired = True, canRepeat = False, mightPack = False, typeCode = FieldType {getFieldType = 9}, typeName = Nothing, hsRawDefault = Nothing, hsDefault = Nothing}], descOneofs = fromList [], keys = fromList [], extRanges = [], knownKeys = fromList [], storeUnknown = False, lazyFields = False, makeLenses = False}"
